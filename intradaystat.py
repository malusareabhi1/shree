...
def load_data(symbol, start, end, interval):
    try:
        ...
        # Fix multi-index and capitalize columns
...
if not df.empty:
    if selected_strategy == 'Opening Range Breakout (ORB)':
        ...
        if 'Volume' in df.columns:
            ...
            st.warning("VWAP strategy skipped: Volume data not available.")
...
# ------------------------------- Display
if df.empty:
    st.error("No data loaded or available for selected interval.")
else:
