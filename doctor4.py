...
if uploaded_file:
    try:
        ...
        if not all(col in df.columns for col in required_cols):
            ...
            st.subheader("📈 Candlestick Chart with Entry/Exit Points")
            ...
            # Entry/Exit from trade_log
            if not trade_log.empty:
                fig.add_trace(go.Scatter(
                    x=pd.to_datetime(trade_log['Entry Time']),
                    y=trade_log['Entry Price'],
                    mode='markers+text',
                    marker=dict(color='green', size=10, symbol='circle'),
                    text=['Entry'] * len(trade_log),
                    textposition='top center',
                    name='Trade Entry'
                ))

                fig.add_trace(go.Scatter(
                    x=pd.to_datetime(trade_log['Exit Time']),
                    y=trade_log['Exit Price'],
                    mode='markers+text',
                    marker=dict(color='red', size=10, symbol='x'),
                    text=['Exit'] * len(trade_log),
            ...
            st.download_button(
                ...
                file_name="dOCTOR_trade_log.csv",
                mime="text/csv"
            )
